...
class Trainer:
    ...
    def Train(self):
        ...
        if self.opt.model_path != 'ACDNet':
            ...
            net = models.GetCNN1DModel().to(self.opt.device);
        ...
        for epochIdx in range(self.opt.nEpochs):
            ...
            for batchIdx in range(n_batches):
                ...
                # Comment line below to not include autoencoder
                with torch.no_grad():
                    x = enc(x);
            ...
            net.eval();
            val_acc, val_loss = self.__validate(net,enc, lossFunc);
            #Save best model
    ...
    def __validate(self, net,enc, lossFunc):
        ...
        with torch.no_grad():
            ...
            for idx in range(math.ceil(len(self.testX)/batch_size)):
                ...
                # Comment line below to not include autoencoder
                x = enc(x);
                # output = torch.reshape(output,(output.shape[0],1,2751));
                # x = torch.reshape(x,(x.shape[0],x.shape[1],1,x.shape[-1]))
                scores = net(x);
    ...
    def __save_model(self, acc, epochIdx, net):
        if acc > self.bestAcc:
            ...
            torch.save({'weight':net.state_dict()}, fname.format(dir, self.opt.model_name.lower(), self.opt.split));
