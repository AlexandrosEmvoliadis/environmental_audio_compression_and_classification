...
def main():
    ...
    for sr in sr_list:
        if sr == 44100:
            ...
            convert_sr(os.path.join(esc50_path, 'ESC-50-master', 'audio'),
                       os.path.join(esc50_path, 'wav{}'.format(sr // 100)),
                       sr);
    ...
    for sr in sr_list:
       if sr == 44100:
           ...
           src_path = os.path.join(esc50_path, 'wav{}'.format(sr // 100));

       create_dataset(src_path,
                      os.path.join(esc50_path, 'wav{}.npz'.format(sr // 100)));
...
def create_dataset(src_path, esc50_dst_path):
    ...
    for fold in range(1, 6):
        ...
        for wav_file in sorted(glob.glob(os.path.join(src_path, '{}-*.wav'.format(fold)))):
            ...
            if opts.binary:
                pol_labels = [40,41,48,43,47,42,45,44,49]
                npol_labels = [19,5,0,11,14,7,16,13]
                if label in pol_labels:
                    label = 0
                elif label in npol_labels:
                    label = 1
                else:
                    continue
