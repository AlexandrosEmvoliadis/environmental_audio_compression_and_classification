...
if __name__=='__main__':
    ...
    # set opt.batchSize to 1600 or lower for PR/NPR Classification
    opt.batchSize=4000;

    for sr in [22050, 10000]:
        opt.sr = sr;
        opt.inputLength = 22050 if sr == 22050 else 30225;
        mainDir = os.getcwd();
        test_data_dir = os.path.join(mainDir, 'datasets/esc50/test_data_{}khz'.format(sr//100));
        print(test_data_dir)
        ...
        val_sounds = [];
        val_labels = [];
        dataset = np.load(os.path.join(opt.data, opt.dataset, 'wav{}.npz'.format(opt.sr // 100)), allow_pickle=True);
        for s in opt.splits:
